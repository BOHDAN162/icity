...
/* ПОРОГИ ХОДА ВПЕРЁД — ИХ ДВА, И ОНИ РАЗНЫЕ НАРОЧНО.
   Просьба заказчика 5 сентября 2026: сначала сайт показывает помещение,
   и только потом зовёт дальше. Числа за день прошли 3 → 4 → нынешние.

     READY_ZONES     2 — индикатор «Листайте вниз» в зонах. Ресепшн
                         и одна зона из плана: зритель уже понял, что
                         зоны переключаются, и пора сказать, что ниже
                         тоже что-то есть.
     PLAN_READY_ZONES 3 — «Дальше» в шапке плана. На единицу больше:
                         из плана ход ведёт мимо остальных зон, и
                         предлагать его тому, кто видел одну, рано.

   Оба считаются по одному и тому же `seen`, чтобы «просмотрено» значило
   одно и то же в обоих местах. */
const READY_ZONES = 2;
const PLAN_READY_ZONES = 3;
    ...
    >
      ...
      <div className={styles.ui}>
        ...
        <div className={styles.bottom} data-side={zone.side === 'right' ? 'right' : 'left'}>
          <div className={`${styles.info} ${zone.side === 'right' ? styles.infoRight : ''}`} key={zoneId}>
            ...
            {/* СЧЁТЧИК ПРОСМОТРЕННЫХ ЗОН. Снимался 5 сентября 2026 вместе
                с кругами перехода и в тот же день возвращён по просьбе
                заказчика: без него зритель не знает ни сколько зон всего,
                ни сколько он уже прошёл, — а обход теперь идёт через
                план, где эта цифра ниоткуда не видна.

                СЧИТАЕТ ПРОСМОТРЕННЫЕ, А НЕ ПОЗИЦИЮ. Навигация графовая,
                а не линейная: «третья из пяти» тут вымысел, а
                «просмотрено три из пяти» — факт, и он же доводит
                зрителя до конца обхода.

                aria-live — здесь же: имя зоны меняется молча, и без
                живой области скринридер не скажет, что зритель куда-то
                перешёл. Отдельной строки под это больше нет. */}
            <p className={styles.zoneCount} aria-live="polite">
              ...
              <span className={`label ${styles.zoneCountLabel}`}>
                {seen.size === ZONE_TOTAL ? 'Все зоны просмотрены' : 'Просмотрено'}
              </span>
            </p>
          </div>
      ...
      {planOpen && (
        <PlanDollhouse
          ...
          nextReady={planReady}
          /* Сигнал приходит в момент клика по «Закрыть», а не в конце
             нырка: угловой индикатор гаснет и приглашение по центру
             встаёт ещё под оверлеем, и растворение открывает уже
             перестроенный экран. */
          onCloseStart={() => setClosedPlan(true)}
        />
      )}
    </section>
  );
