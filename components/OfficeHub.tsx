...
/* iCITY 113Н — офис. Остановка сразу после подъёма вдоль башни.
   ...
   Сброс на ресепшн делает закрытие плана — см. closePlan() ниже.
...
export default function OfficeHub({ active }: Props) {
  ...
  /* Esc и «Закрыть» без выбора зоны ведут на ресепшн — заказчик попросил
     именно так. Но PlanDollhouse зовёт этот же onClose ещё и ПОСЛЕ
     удачного выбора двери (там уже отработал onEnterZone → go(),
     и офис уже переключён на выбранную зону) — с флагом `picked`.
     Без разбора по этому флагу выбор зоны внутри плана тут же
     перезаписывался бы ресепшном: этот баг уже успел долететь
     до заказчика. */
  const closePlan = useCallback((picked: boolean) => {
    setPlanOpen(false);
    if (!picked) go('reception');
  }, [go]);
  ...
  /* Глобального Esc здесь больше нет: на ресепшне и в зонах Esc не делает
     ...
     см. closePlan() выше, он же возвращает зрителя на ресепшн. */

  /* Секция ушла из кадра с открытым планом (например, свернули вкладку
     и вернулись уже ниже) — план закрываем и так же уводим на ресепшн,
     а не оставляем висеть поверх чужой секции. При обычном закрытии
     этот путь не участвует: скролл заперт эффектом выше, пока план жив.
  ...
  if (wasActive !== active) {
    setWasActive(active);
    if (!active && planOpen) { setPlanOpen(false); go('reception'); }
  }
  ...
  return (
    ...
    >
      ...
      {planOpen && <PlanDollhouse onClose={closePlan} onEnterZone={go} />}
    </section>
  );
}
