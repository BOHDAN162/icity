...
/* ПОРОГ ХОДА ВПЕРЁД — ТРИ ПРОСМОТРЕННЫЕ ЗОНЫ СЧИТАЯ РЕСЕПШН.
   Просьба заказчика 5 сентября 2026: сначала сайт показывает помещение,
   и только потом зовёт дальше. Путь ровно такой — ресепшн, зона из
   плана, ещё зона из плана; к третьей зритель видел достаточно, чтобы
   приглашение листать не выглядело попыткой его спровадить.
...
export default function OfficeHub({
  ...
  useEffect(() => {
    ...
    warm();
    return onScrollLockChange(warm);
  }, [active]);
