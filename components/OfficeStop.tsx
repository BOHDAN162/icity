...
export default function OfficeStop() {
  ...
  useEffect(() => {
    ...
    /* Кнопка «Листайте вниз» в офисе просит ровно тот же шаг, что и жест,
       но признак цепочки поднимает: зритель пришёл на кадр КЛИКОМ,
       и приглашение листать должно встретить его и там — а после
       клика по нему и на втором кадре. Жест этот признак снимает
       (см. arm): крутанул колесо — значит листает сам. */
    const offCue = onNextStep(() => {
      if (!parked) return;
      stage.dataset.cue = '1';
      runStep(Math.min(cur + 1, 2) as Step);
    });
