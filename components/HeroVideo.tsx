...
/** сколько амбиент тихо доигрывает уже поверх экрана офиса
    ...
    БЫЛО 4000 И СОКРАЩЕНО ДО 1000 — вынужденно. Столько материала у трека
    после свапа просто нет: он 10,51 с при ролике 9,79 с, то есть остаётся
    0,7 с. Раньше недостачу закрывало зацикливание, и оно же дало худший
    баг первого экрана: на телефоне после свапа шли пара секунд тишины,
    а потом амбиент начинал играть заново, с начала трека. Заказчик ловил
    это трижды. Зацикливания больше нет вовсе (см. startAmbienceTail),
    значит и хвост не может быть длиннее собственного окончания трека. */
...
const startAmbienceTail = (a: HTMLAudioElement, g: AmbienceGain | null) => {
  /* ЗАЦИКЛИВАНИЯ ЗДЕСЬ НЕТ И БОЛЬШЕ НЕ БУДЕТ. Оно стояло затем, что
     материала после свапа остаётся 0,7 с, а хвост просили длиннее, — и
     оно же трижды дало заказчику одну и ту же беду на iPhone: секунда-две
     тишины после открытия офиса, а потом амбиент начинает играть заново,
     с тихого начала трека и с новым подъёмом. Прятать шов затуханием
     оказалось нечем: любая ветка, где затухание не доходит до выхода,
     возвращает беду целиком. Хвост теперь ровно такой, какой у трека
     остался, и кончается насмерть.

     СНЯТИЕ ТЕРМИНАЛЬНОЕ, И ЭТО ГЛАВНОЕ. Мало поставить на паузу: элемент
     живёт вне React, и любое чужое возобновление (движок, восстановление
     аудиосессии после конца видео, что угодно) снова дало бы звук поверх
     офиса. Поэтому у элемента отбираются источники и дёргается load():
     после этого играть ему физически нечего. */
  ...
  const stop = () => {
    ...
    a.pause();
    /* Источники долой: сначала <source>, потом src, потом load() —
       алгоритм выбора ресурса стартует заново, не находит ничего
       и обрывает воспроизведение окончательно. */
    a.replaceChildren();
  ...
  /* Трек кончится сам примерно через 0,7 с — снимаем сразу, не дожидаясь
     предохранителя. */
  a.addEventListener('ended', stop, { once: true });
  /* Предохранитель обязателен и теперь ещё и короткий: rAF в фоновой
     вкладке встаёт, а звук поверх офиса не должен пережить свап
     ни при каком раскладе. */
  guard = window.setTimeout(stop, AUDIO_TAIL_MS + 300);
  const step = () => {
