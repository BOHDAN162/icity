...
SRC, DST = 'geom_v11_data.json', 'geometry.json'
PANE     = 2.00
GAP      = 0.08
SLOT     = 0.14
SIMP     = 0.004
...
G['notes'] = ('v11: пять внутренних перегородок стали стеклом (walls_ortho '
              '-> glazing_interior): запад Skype-блока, стык Skype-комнат, '
              'обе стены переговорной, западная стена кабинета восток. '
              'У стыка Skype-комнат заодно убран дубликат в glazing_interior '
              '(тот же кусок стены был обмерен дважды, расхождение 2 см — '
              'подозреваемая причина диагонального артефакта). Контур плиты '
              'и остальные стены — без изменений от v10.')
