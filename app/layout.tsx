...
/* viewport-fit: cover СНЯТ, и это не откат ради отката.
   С ним страница раскладывается на весь физический экран и уходит ПОД
   панель Safari и под домашний индикатор. Задумано это ради полноэкранного
   кадра, а на деле дало ровно одну вещь, которую заказчик ловил четыре
   раза подряд: полосу в самом низу. Под панелью то виден кусок следующей
   секции, то фотография комплекса, то фон канвы, — и что бы там ни
   оказалось, оно выглядит грязным краем, потому что панель полупрозрачная
   и подмешивает к себе то, что под ней.

   Без cover видимая область и есть вся раскладка: под панель не уезжает
   ничего, safe-area браузер закрашивает фоном страницы сам. Кадр первого
   экрана от этого ничего не теряет — он и так занимал ровно видимую
   часть, композиция в нём считается от dvh.

   Побочно это чинит и всю арифметику высот: lvh, dvh и svh начинают
   мериться от области, в которой мы реально рисуем. */
